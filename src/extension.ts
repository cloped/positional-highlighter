...
export function activate(context: vscode.ExtensionContext) {
  const command = vscode.commands.registerCommand(
    ...
    () => {
      ...
      const rules: Rule[] = vscode.workspace
        .getConfiguration("positionalHighlighter")
        .get("rules", []);
              ...
              (range: { start: number; end: number }, index: number) => {
                ...
                if (formattedLineText.length > range.start) {
                  ...
                  const endPos = new vscode.Position(
                    lineIndex,
                    Math.min(range.end + 1, lineText.length),
                  );
                  allDecorations[color].push(
